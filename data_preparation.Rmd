...
df_budget_transparency_index <- df_all_yr %>% 
  ...
  distinct() %>%
  mutate(
    ogp_transparency_fisc_open_noNA = ogp_transparency_fisc_open,
    ...
    budget_transparency_index_noNA = rescale(ogp_transparency_fisc_open_noNA)) %>%
...
histogram(df_budget_transparency_index$budget_transparency_index_noNA)

df_budget_transparency_index %>% count(budget_transparency_index)
df_budget_transparency_index %>% count(budget_transparency_index_noNA)
...
df_budget_particpation_index <- df_all_yr %>%
  ...
  distinct() %>%
  mutate(
    ogp_participation_fisc_open_noNA = ogp_participation_fisc_open,
    ...
    budget_participation_index_noNA = rescale(ogp_participation_fisc_open_noNA)) %>%
  select(country_standard, budget_participation_index, budget_participation_index_noNA, everything()) %>%
  arrange(country_standard)

skim(df_budget_particpation_index)
histogram(df_budget_particpation_index$budget_participation_index)
histogram(df_budget_particpation_index$budget_participation_index_noNA)

df_budget_particpation_index %>% count(budget_participation_index)
df_budget_particpation_index %>% count(budget_participation_index_noNA)
...
df_indices_controls <- df_transparency_index %>%
  ...
  full_join(df_budget_particpation_index[ , c("country_standard", "budget_participation_index", "budget_participation_index_noNA", "incomplete_bdgt_part_index")], by = "country_standard") %>%
  full_join(df_budget_transparency_index[ , c("country_standard", "budget_transparency_index", "budget_transparency_index_noNA", "incomplete_bdgt_trans_index")], by = "country_standard") %>%
...
# Creating index for cross-section data set for regressions

df_indices_controls_cross_section <- df_indices_controls %>% 
  filter(year<2020) %>% # There is no transparency index data in 2020
  select(country_standard:transparency_index, budget_participation_index:aged_65_older) %>%
  # Removing rows with all NA values for pandemic violations index or covid index
  filter(
    if_any(
      budget_participation_index:covid_index, 
      ~ !is.na(.))) %>%
  group_by(country_standard) %>%
  mutate(
    transparency_index_mean = mean(transparency_index, na.rm = TRUE)) %>%
  ungroup() %>%
  arrange(country_standard, year) %>%
  group_by(country_standard) %>%
  # We'll be keeping only 2019 data, and if the transprency index is missing 
  #   that year, it'll be filled in with the most recent available value
  fill(transparency_index) %>%
  ungroup() %>%
  rename(transparency_index_2019 = transparency_index) %>%
  filter(year==2019) %>%
  select(country_standard, transparency_index_2019, transparency_index_mean, 
           budget_participation_index:aged_65_older) %>%
  distinct()

#df_indices_controls_cross_section %>% skim()    

write_csv(df_indices_controls_cross_section, "data/indices_and_controls_cross_section.csv")
