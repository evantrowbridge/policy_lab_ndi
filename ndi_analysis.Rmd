...
#   Also, the most recent data in the index is from 2019, but to make it
#   easier to run regressions with COVID-19 data, we copy the 2019 data in
...
df_transparency_index <- df_transparency_indicators %>%
  ...
  mutate(
    across(
      fh_global_freedom_score:vdem_horizontal_account,
      ~rescale(., to = c(1,10))),
  transparency_index = rescale(
    fh_global_freedom_score +
    wgi_voice_and_accountability +
    vdem_diagonal_account +
    vdem_vertical_account +
  ...
  arrange(country_standard, year) %>%
  group_by(country_standard) %>%
  # Using 2019 index values for 2020 and 2021
  mutate(
    transparency_index = case_when(
      year == 2020 ~lag(transparency_index),
      year == 2021 ~lag(transparency_index, n = 2),
      TRUE~transparency_index
    )
  ) %>%
  ungroup()
...
#   This index also uses "fill" between missing dates
...
df_trust_index <- df_trust_indicators %>%
  ...
  mutate(
    across(
      eiu_electoral:wvs_trust_elect,
      ~rescale(., to = c(1,10))),
    trust_index_no_elec = rescale(
      eiu_electoral +
      wvs_trust_parliament +
      wvs_trust_gov +
      wvs_trust_court),
...
df_corruption_indicators$wgi_control_of_corruption <- as.character(df_corruption_indicators$wgi_control_of_corruption)
df_corruption_indicators$wgi_control_of_corruption <- as.numeric(df_corruption_indicators$wgi_control_of_corruption)

#rescaling and creating index of corruption (cpi_score: 0-100, WGI: -2.5~2.5)
corruption_index_pre <- df_corruption_indicators %>%
  mutate(wvs_corruption_scale = rescale(wvs_corruption)) %>% # Evan added this line and modified lines below
  mutate(df_corruption_indicators, cpi_score_scale = df_corruption_indicators$cpi_score/100) %>%
  mutate(df_corruption_indicators, wgi_coc_scale = (df_corruption_indicators$wgi_control_of_corruption+2.5)/5) %>%
  mutate(df_corruption_indicators, corruption_idx_sum = ifelse(year >= 2012, cpi_score_scale + wgi_coc_scale + wvs_corruption_scale, NA )) %>%
  mutate(df_corruption_indicators, corruption_idx_sum_no_wvs = ifelse(year >= 2012, cpi_score_scale + wgi_coc_scale, NA )) %>%
  mutate(corruption_index = ifelse(
      is.na(corruption_idx_sum),
            corruption_idx_sum_no_wvs,
            corruption_idx_sum)) %>%
  mutate(df_corruption_indicators, corruption_idx_avg = ifelse(year >= 2012, (cpi_score_scale + wgi_coc_scale)/2, NA ))

#   Also, the most recent data in the index is from 2019, but to make it
#   easier to run regressions with COVID-19 data, we copy the 2019 data in
#   the years 2020 and 2021.

df_corruption_index <- corruption_index_pre %>%
  select(country_standard, year, corruption_index, cpi_score_scale, wgi_coc_scale, wvs_corruption_scale) %>%
  arrange(country_standard, year) %>%
  distinct() %>%
  group_by(country_standard) %>%
  # Using 2019 index values for 2020 and 2021
  mutate(
    corruption_index = case_when(
      year == 2020 ~lag(corruption_index),
      year == 2021 ~lag(corruption_index, n = 2),
      TRUE~corruption_index
    )
  ) %>%
  ungroup()
...
df_pandem_dem_violation_index <- df_all_yr %>%
  ...
  select(country_standard, year, pandemic_dem_violation_index, everything())
...
df_indices_controls <- df_transparency_index %>%
  select(country_standard, year, transparency_index) %>%
  full_join(df_trust_index[ , c("country_standard", "year", "trust_index")], by = c("country_standard", "year")) %>%
  full_join(df_corruption_index[ , c("country_standard", "year", "corruption_index")], by = c("country_standard", "year")) %>%
  full_join(df_effectiveness_index[ , c("country_standard", "year", "effectiveness_index")], by = c("country_standard", "year")) %>%
  full_join(df_budget_particpation_index[ , c("country_standard", "year", "budget_participation_index")], by = c("country_standard", "year")) %>%
  full_join(df_pandem_dem_violation_index[ , c("country_standard", "year", "pandemic_dem_violation_index")], by = c("country_standard", "year")) %>%
