...
packages <- c("tidyverse", "readxl", "janitor", "reticulate", "countrycode", "Hmisc", "lubridate", "skimr", "haven")
...
## Data from Open Government Partnership ##
  ...
  df_ogp <- import_OGP_data %>%
    ...
    filter(!country_code %in% c(
      'ARBA', 'BRSP', 'CAON', 'CONR', 'ESMD', 'ESPV', 'FRPA', 'GBSC', 'GETB', 
      'GHST', 'IDBO', 'KEEM', 'KRSE', 'MXJL', 'NGKD', 'PELL', 'PHSC', 'ROIS', 
      'TZKG', 'USAU')) %>%    
    mutate(
      country_standard = countrycode(
        country_code,
...
## WGI indicators ##
  ...
  df_wgi <- import_Worldwide_Governance_Indicators_clean %>%
    ...
    rename_with( 
      ~ paste("wgi", .x, sep = "_"),
      voice_and_accountability_x:voice_and_accountability_y) %>%
...
## Africa Integrity Indicators ##
  ...
  names(df_africa_int) <- names(df_africa_int) %>% str_remove("^in_")
  names(df_africa_int) <- abbreviate(names(df_africa_int), minlength = 8)
...
## Public health expenditures as share of GDP   
  ...
  #label(df_gdp_health_expend$pub_health_exp_percent_gdp) <- "Public expenditure on health as a percent of GDP"
  
  # df_gdp_health_expend$pub_health_exp_percent_gdp <- labelled(
  #   df_gdp_health_expend$pub_health_exp_percent_gdp,
  #   label = "Public expenditure on health as a percent of GDP")
...
A helpful link for issues: https://github.com/ropensci/skimr/issues/606

```{r}
skim_all <- skim(df_all)
skim_all_yr <- skim(df_all_yr)
