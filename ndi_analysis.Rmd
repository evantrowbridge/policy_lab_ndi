...
## Freedom House indicators ##
  ...
  df_fh <- import_Freedom_house %>%
    ...
    #            'Transnistria', 'Chechnya', 'Serbia and Montenegro')) %>%
...
## Gini Coefficient ##
  ...
  # Gini data is only up to date through 2018
  import_Gini_coefficient <- WDI(indicator = 'SI.POV.GINI', start = 2006, end = 2018)
...
## GDP Per Capita ##
  ...
  #   We are using GDP Per Capita, Purchasing Power Parity in current international $
  import_gdp_percap <- WDI(indicator = 'NY.GDP.PCAP.PP.CD', start = 2006, end = 2019)
  
  # If we wanted to use GDP per capita in current $US (not PPP), can use line below instead
  # import_gdp_percap<- WDI(indicator = 'NY.GDP.PCAP.CD', start = 2006, end = 2019)
  ...
  df_gdp_percap <- import_gdp_percap %>%
    ...
    rename(gdp_percap = ny_gdp_pcap_pp_cd) %>%
    # Line below is for non-PPP GDP per capita
    # rename(gdp_percap = ny_gdp_pcap_cd) %>%
    filter(!is.na(country_standard)) %>%
    select(country_standard, year, gdp_percap)  
...
```{r merging_data, include=FALSE}
  ...
  df_all_yr <- df_covid_owid_yr %>%
    ...
    select(
      ...
      pub_health_exp_percent_gdp,
      everything(),
      # We import separate GDP per cap below directly from WB
      -gdp_per_capita,
      -code.x,
      -entity
      ) %>%
...
df_controls <- df_all_yr %>%
  ...
  fill(
    c(gdp, gini, percap_domestic_health_expenditure, gdp_percap)) %>%
  filter(year==2020) %>%
  select(
    country_standard, 
    gdp,
...
df_controls <- df_all_yr_2020 %>%
  ...
  select(
    country_standard, gdp, gini, gdp_percap,
